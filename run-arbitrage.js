...
const web3 = new Web3(
    new Web3.providers.WebsocketProvider(process.env.INFURE_URL)
);
...
const init = async () => {
    ...
    web3.eth.subscribe('newBlockHeaders')
        .on('data', async block => {
            ...
            const [tx1, tx2] = Object.keys(DIRECTION).map(direction => flashloan.methods.initiateFlashloan(addresses.dydx.solo, addresses.tokens.dai, AMOUNT_DAI_WEI, DIRECTION[direction]));
            const [gasPrice, gasCost1, gasCost2] = await Promise.all([web3.eth.getGasPrice(), tx1.estimateGas({from: admin}), tx2.estimateGas({from: admin})]); 
